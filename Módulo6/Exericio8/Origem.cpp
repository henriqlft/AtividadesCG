...
vector<GLfloat> posicoes;
vector<GLfloat> normais;
vector<GLfloat> coordTextura;
vector<GLfloat> ka;
vector<GLfloat> ks;
float ns;
string mtlFile = "";
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Atividade Modulo 6 - Adicionando Trajetoria - Henrique Lessinger Feiten", nullptr, nullptr);
	...
	loadOBJ("C:/Users/I521889/Documents/GitHub/AtividadesCG/3D_Models/Suzanne/CuboTextured.obj");
	loadMTL("C:/Users/I521889/Documents/GitHub/AtividadesCG/3D_Models/Suzanne/CuboTextured.mtl");
	GLuint textureID = loadTexture("C:/Users/I521889/Documents/GitHub/AtividadesCG/3D_Models/Suzanne/Cube.png");
	...
	Mesh suzanne;
	suzanne.initialize(VAO, posicoes.size() / 3, &shader, textureID);
...
int setupGeometry()
{
	...
	glBufferData(GL_ARRAY_BUFFER, posicoes.size() * sizeof(GLfloat), posicoes.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);

	glBindBuffer(GL_ARRAY_BUFFER, VBO[1]);
	glBufferData(GL_ARRAY_BUFFER, coordTextura.size() * sizeof(GLfloat), coordTextura.data(), GL_STATIC_DRAW);
	...
	while (getline(file, line))
	{
		...
		else if (prefix == "f")
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				posicoes.push_back(vertex.x);
				posicoes.push_back(vertex.y);
				posicoes.push_back(vertex.z);
				coordTextura.push_back(texture.x);
				coordTextura.push_back(texture.y);
